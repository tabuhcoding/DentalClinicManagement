...
--bỏ mức cô lập read uncommited, dùng mức cô lập mặc định của SQL, read commited
BEGIN TRAN
...
CREATE OR ALTER
PROCEDURE SP_PHANTOMREAD_1
	@Name nvarchar(30)
AS
SET TRAN ISOLATION LEVEL SERIALIZABLE --giữ shared lock đến hết giao tác, nên các giao tác khác phải chờ nếu muốn cập nhật, thay đổi giá trị
BEGIN TRAN
	SELECT *
...
----------
-- HUY - Cài đặt XỬ LÝ tình huống tranh chấp: Lost Update
...
--THI-------------------------------------------------------
--Dirty read
CREATE PROC SP_DIRTYREAD_FIX_THI
	@AccountID INT,
	@NewPhone CHAR(10)
AS
BEGIN TRAN
...
CREATE PROC SP_DIRTYREAD_FIX_2_THI
	@AccountID INT
AS
--SET TRAN ISOLATION LEVEL READ UNCOMMITTED
BEGIN TRAN
...
--Unrepeatable Read
CREATE PROC SP_UNREPEAT_FIX_THI
		@AccountID INT
AS
SET TRAN ISOLATION LEVEL REPEATABLE READ
BEGIN TRAN
...
CREATE PROC SP_UNREPEAT_FIX_2_THI
		@AccountID INT
AS
BEGIN TRAN
	BEGIN TRY
