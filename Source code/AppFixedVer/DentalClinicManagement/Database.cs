...
namespace DentalClinicManagement
{
    public class DB
    {
        ...
        public string ConnectionString { get; set; } = "Server=DESKTOP-MQMBQC9;Database=DentalClinicManagement;Integrated Security=True;MultipleActiveResultSets=True;";
        ...
        public static int MakeAppointment(DentistAvailableTime dat, int? cusID, string note)
        {
            using (SqlConnection connection = new SqlConnection(DB.Instance.ConnectionString))
            {
                ...
                if (rs == 0)
                {
                    ...
                    // Assuming this method reloads data after a successful appointment
                }
                else
                {
                    Console.WriteLine("Error making appointment.");
                }
                return rs;
            }
        }
